...
export default function ThereminCanvas({
  ...
  const [leftCentroid, setLeftCentroid] = useState<{
    x: number;
    y: number;
  } | null>(null);
  const [rightCentroid, setRightCentroid] = useState<{
    x: number;
    y: number;
  } | null>(null);
    ...
    const initMediaPipe = async () => {
      try {
        ...
        try {
          ...
          console.error(
            "Failed to initialize mediapipe (CPU fallback also failed):",
            e2,
          );
        }
      }
        ...
        if (video.currentTime !== lastVideoTime) {
          ...
          if (detectedLeftHand) {
            ...
            leftHandRef.current.x +=
              (targetLX - leftHandRef.current.x) * smoothFactor;
            leftHandRef.current.y +=
              (targetLY - leftHandRef.current.y) * smoothFactor;
          ...
          if (detectedRightHand) {
            ...
            rightHandRef.current.x +=
              (targetRX - rightHandRef.current.x) * smoothFactor;
            rightHandRef.current.y +=
              (targetRY - rightHandRef.current.y) * smoothFactor;
            rightHandRef.current.ticksSinceLastMotion = 0;
            rightHandRef.current.active = true;
            setRightCentroid({
              x: detectedRightHand.x,
              y: detectedRightHand.y,
            });
          } else {
          ...
          if (
            ...
            if (leftHandRef.current.ticksSinceLastMotion < maxHoldTicks) {
              ...
              leftVolMult =
                1.0 -
                (leftHandRef.current.ticksSinceLastMotion - maxHoldTicks) /
                  fadeoutDuration;
            }
          } else {
            setLeftCentroid(null);
          ...
          if (
            ...
            if (rightHandRef.current.ticksSinceLastMotion < maxHoldTicks) {
              ...
              rightVolMult =
                1.0 -
                (rightHandRef.current.ticksSinceLastMotion - maxHoldTicks) /
                  fadeoutDuration;
            }
          } else {
            setRightCentroid(null);
          ...
          if (isActivated) {
            if (leftActive && rightActive) {
              ...
              const rootOctaveFreq =
                65.4 * Math.pow(2, settings.baseOctave - 2);
              ...
              if (settings.responseMode === "log") {
                targetFreq =
                  minFreq * Math.pow(maxFreq / minFreq, activePitchRatio);
              } else {
                targetFreq = minFreq + (maxFreq - minFreq) * activePitchRatio;
              }

              globalAudioEngine.startOscillator();
              const finalParam = globalAudioEngine.updateParameters(
                targetFreq,
                activeVolumeRatio,
              );
              if (finalParam) {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (true) {
        ...
        const pitchRatio =
          lastAssignedFreq > 0 ? Math.log2(lastAssignedFreq / 110) : 1;
        ...
        for (let l = 0; l < numLayers; l++) {
          ...
          if (l === 0) {
            ctx.shadowColor = getThemeColor(
              true,
              isPointerInRef.current ? 0.6 : 0.1,
            );
            ctx.shadowBlur = isPointerInRef.current
              ? 15 + currentAmplitude * 10
              : 0;
          } else {
            ctx.shadowBlur = 0;
          }
          ...
          const maxAmpPx =
            canvas.height * 0.25 * currentAmplitude * (1.0 - l * 0.15);
          ...
          for (let x = 0; x <= canvas.width; x += 5) {
            ...
            const w1 =
              Math.sin(nx * Math.PI * waveCount * 2.0 + phaseOffset) * 0.5;
            const w2 =
              Math.sin(nx * Math.PI * waveCount * 1.3 - phaseOffset * 0.7) *
              0.3;
            const w3 =
              Math.sin(nx * Math.PI * waveCount * 0.8 + phaseOffset * 1.1) *
              0.2;
  ...
  const handlePointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    handleInteraction(
      e.clientX,
      e.clientY,
      isTouch || isPointerDownRef.current,
    );
  };
  ...
  return (
    ...
    >
          ...
          <div className="relative w-24 h-24 md:w-36 md:h-36 rounded-full overflow-hidden border border-white/10 shadow-lg bg-black/40 backdrop-blur-md flex items-center justify-center">
            ...
            {!leftCentroid && !rightCentroid && !cameraError && (
              <div className="absolute inset-0 pointer-events-none">
                ...
                <div
                  className="absolute inset-0 rounded-full border-2 border-transparent border-t-[#44e2cd]/70 animate-spin"
                  style={{ animationDuration: "1.5s" }}
                />
                <div
                  className="absolute inset-[4px] rounded-full border border-transparent border-b-[#cebdff]/40 animate-spin"
                  style={{
      ...
      {/* Active Web camera activation toggle button — top-right, below telemetry strip */}
      <div className="absolute top-[10rem] right-4 md:top-28 md:right-12 z-50 pointer-events-auto flex items-center gap-2">
        {cameraError && (
